...
class EventContent(ctk.CTkFrame):
    ...
    def setup(self) -> None:
        self.validate_command_2_dig = self.register(lambda new_value: validate_input(new_value, 2))
        self.validate_command_4_dig = self.register(lambda new_value: validate_input(new_value, 4))
        self.content_title()
        self.content_date()
        self.content_time()
        self.content_notes()
    ...
    # date
        date_day: str = self.event_date_day.get()
        date_month: str = self.event_date_month.get()
        date_year: str = self.event_date_year.get()
    # time
        time_hour: str = self.event_time_hour.get()
        time_minutes: str = self.event_time_minutes.get()
    # notes
        notes: str = self.event_notes.get('0.0', 'end')
    # all together
        self.original_info = f'{title},{date_day}.{date_month}.{date_year},{time_hour}:{time_minutes},{notes[:-1]}'

    def content_title(self) -> None:
    # title frame
        title_frame: ctk.CTkFrame = ctk.CTkFrame(self, fg_color=Colors.MAIN_COLOR)
        title_frame.pack(side=ctk.TOP, padx=15, pady=15, fill=ctk.X)
    # check box
        check_box: ctk.CTkCheckBox = ctk.CTkCheckBox(title_frame, text='', width=0, height=0)
        check_box.pack(side=ctk.LEFT, padx=0, pady=0, anchor=ctk.W, fill=ctk.X)
    # title
        self.title: ctk.CTkEntry = ctk.CTkEntry(title_frame, fg_color=Colors.MAIN_COLOR,
    ...
    # date frame
        date_frame: ctk.CTkFrame = ctk.CTkFrame(self, fg_color=Colors.MAIN_COLOR)
        date_frame.pack(side=ctk.TOP, padx=40, pady=5, anchor=ctk.W)
    # date label
        label_title: ctk.CTkLabel = ctk.CTkLabel(date_frame, fg_color=Colors.MAIN_COLOR,
                                    text_color=Colors.TEXT_COLOR, text='Date: ',
                                    width=50, corner_radius=0,
                                    font=ctk.CTkFont('Comic Sans', 32))
        label_title.pack(side=ctk.LEFT, padx=2, pady=2)
    # day
        self.event_date_day: ctk.CTkLabel = ctk.CTkEntry(date_frame, fg_color=Colors.MAIN_COLOR,
                                border_color=Colors.BACKGROUND, width=50, corner_radius=0,
                                font=ctk.CTkFont('Comic Sans', 32), justify=ctk.RIGHT,
                                text_color=Colors.TEXT_COLOR, validate='key',
                                validatecommand=(self.validate_command_2_dig, '%P'))
        date_day: str = f'{self.event.split(',',2)[1].split('.',1)[0]}'
        self.event_date_day.insert('0', date_day)
        self.event_date_day.pack(side=ctk.LEFT, padx=2, pady=2)
    # separator
        separator_0: ctk.CTkLabel = ctk.CTkLabel(date_frame, text_color=Colors.TEXT_COLOR, text='/', font=ctk.CTkFont('Comic sans', 32))
        separator_0.pack(side=ctk.LEFT, padx=0, pady=0, anchor=ctk.W)
    # month
        self.event_date_month: ctk.CTkEntry = ctk.CTkEntry(date_frame, fg_color=Colors.MAIN_COLOR,
                                border_color=Colors.BACKGROUND, width=50, corner_radius=0,
                                font=ctk.CTkFont('Comic Sans', 32), justify=ctk.RIGHT,
                                text_color=Colors.TEXT_COLOR, validate='key',
                                validatecommand=(self.validate_command_2_dig, '%P'))
        date_month: str = f'{self.event.split(',',2)[1].split('.',2)[1]}'
        self.event_date_month.insert('0', date_month)
        self.event_date_month.pack(side=ctk.LEFT, padx=2, pady=2)
    # separator
        separator_1: ctk.CTkLabel = ctk.CTkLabel(date_frame, text_color=Colors.TEXT_COLOR, text='/', font=ctk.CTkFont('Comic sans', 32))
        separator_1.pack(side=ctk.LEFT, padx=0, pady=0, anchor=ctk.W)
    # year
        self.event_date_year: ctk.CTkEntry = ctk.CTkEntry(date_frame, fg_color=Colors.MAIN_COLOR,
                                border_color=Colors.BACKGROUND, width=88, corner_radius=0,
                                font=ctk.CTkFont('Comic Sans', 32), justify=ctk.RIGHT,
                                text_color=Colors.TEXT_COLOR, validate='key', validatecommand=(self.validate_command_4_dig, '%P'))
        date_year: str = f'{self.event.split(',',2)[1].split('.',3)[2]}'
        self.event_date_year.insert('0', date_year)
        self.event_date_year.pack(side=ctk.LEFT, padx=2, pady=2)

    def content_time(self) -> None:
    # time frame
        time_frame: ctk.CTkFrame = ctk.CTkFrame(self, fg_color=Colors.MAIN_COLOR)
        time_frame.pack(side=ctk.TOP, padx=40, pady=5, anchor=ctk.W)
    # time label
        label_title: ctk.CTkLabel = ctk.CTkLabel(time_frame, fg_color=Colors.MAIN_COLOR,
                                    text_color=Colors.TEXT_COLOR, text='Time: ', width=50, corner_radius=0,
                                    font=ctk.CTkFont('Comic Sans', 32))
        label_title.pack(side=ctk.LEFT, padx=2, pady=2)
    # hour
        self.event_time_hour: ctk.CTkEntry = ctk.CTkEntry(time_frame, fg_color=Colors.MAIN_COLOR,
                                border_color=Colors.BACKGROUND, width=50, corner_radius=0,
                                font=ctk.CTkFont('Comic Sans', 32), justify=ctk.RIGHT,
                                text_color=Colors.TEXT_COLOR, validate='key', validatecommand=(self.validate_command_2_dig, '%P'))
        time_hour: str = (self.event.split(',',2)[2]).split(':')[0]
        self.event_time_hour.insert('0', time_hour)
        self.event_time_hour.pack(side=ctk.LEFT, padx=5, pady=5, anchor=ctk.W)
    # separator
        separator: ctk.CTkLabel = ctk.CTkLabel(time_frame, text_color=Colors.TEXT_COLOR, text=':',
                                font=ctk.CTkFont('Comic sans', 32))
        separator.pack(side=ctk.LEFT, padx=0, pady=0, anchor=ctk.W)
    # minute 
        self.event_time_minutes: ctk.CTkLabel = ctk.CTkEntry(time_frame, fg_color=Colors.MAIN_COLOR,
                                border_color=Colors.BACKGROUND, width=50, corner_radius=0,
                                font=ctk.CTkFont('Comic Sans', 32), justify='right',
                                text_color=Colors.TEXT_COLOR, validate='key', validatecommand=(self.validate_command_2_dig, '%P'))
        time_minutes: str = (self.event.split(',',3)[2]).split(':')[1]
    ...
    # notes label
        spacing_0: ctk.CTkFrame = ctk.CTkFrame(self, width=1, height=1, corner_radius=0,
    ...
    # notes
        self.event_notes: ctk.CTkTextbox = ctk.CTkTextbox(self, fg_color=Colors.MAIN_COLOR, border_width=2,
                                        border_color=Colors.BACKGROUND, corner_radius=0,
                                        wrap='word', font=ctk.CTkFont('Comic Sans', 32),
                                        text_color=Colors.TEXT_COLOR)
        notes: str = f'{self.event.split(',',3)[3].replace(' /n|', '\n').replace(' /r|', '\r')}'
        self.event_notes.insert('0.0', notes)
        self.event_notes.pack(side=ctk.TOP, padx=50, anchor=ctk.N,
                            fill=ctk.BOTH, expand=True)
    # spacing
        spacing_1: ctk.CTkFrame = ctk.CTkFrame(self, width=1, height=1, corner_radius=0,
                                fg_color=Colors.MAIN_COLOR)
        spacing_1.pack(side=ctk.BOTTOM, padx=50, pady=100)

    def remove_event_button(self) -> None:
        remove_button: ctk.CTkButton = ctk.CTkButton(self.buttons_frame, text_color=Colors.TEXT_COLOR, text=' REMOVE ',
    ...
    def save_edited_event_button(self) -> None:
        remove_button: ctk.CTkButton = ctk.CTkButton(self.buttons_frame, text_color=Colors.TEXT_COLOR, text=' SAVE ',
